...
** Last update Wed Jun 15 12:51:06 2016 Christian Diaconu
*/
...
MapTile *GetTopTile(Drone *this_drone, Map *map)
{
  if (this_drone->rotation == TOP)
    return (Math_GetTopTile(map, this_drone->mapTile)); // OK
  if (this_drone->rotation == RIGHT)
    return (Math_GetRightTile(map, this_drone->mapTile)); // OK
  if (this_drone->rotation == BOTTOM)
    return (Math_GetBottomTile(map, this_drone->mapTile)); // OK
  if (this_drone->rotation == LEFT)
    return (Math_GetLeftTile(map, this_drone->mapTile)); // OK
  return (NULL);
}
